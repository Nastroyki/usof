...
router.post("/register", async (req, res) => {
    try {
        ...
        if (loginCheck.id != 0) {
            ...
            if (loginCheck.password != encryptedPassword) {
                return res.status(409).send("User Already Exist. Please Login");
            }
            else {
                loginCheck.email = email;
                loginCheck.email_code = generateCode();
                loginCheck = await User.save(loginCheck);
                let mailOptions = {
                    from: process.env.MAIL_USER,
                    to: loginCheck.email,
                    subject: 'Your registration code for USOF',
                    text: 'Your registration code is ' + loginCheck.email_code
                };
